...
export default function SafetyBatchUploadDialog({
  open,
  onOpenChange,
  onSuccess,
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        const uploadPromise = cloudInstance.uploadFile({
          cloudPath: `safety_environment_system/batch/${Date.now()}_${i}_${file.name}`,
          filePath: file
        }).then(uploadResult => {
          const fileData = {
            fileName: file.name,
      ...
      <div className="space-y-4 py-4">
        ...
        <div>
          ...
          <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-gray-400 transition-colors">
            ...
            <input type="file" multiple accept=".pdf" onChange={handleFileChange} className="hidden" id="safety-batch-pdf-upload" />
            <label htmlFor="safety-batch-pdf-upload" className="cursor-pointer text-sm text-blue-600 hover:text-blue-800">
              点击选择多个PDF文件
            </label>
