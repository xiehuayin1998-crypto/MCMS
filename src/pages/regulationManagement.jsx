...
// @ts-ignore;
import RegulationUploadDialog from '@/components/RegulationUploadDialog';
// @ts-ignore;
import RegulationBatchUploadDialog from '@/components/RegulationBatchUploadDialog';
// @ts-ignore;
import { UserHeader } from '@/components/UserHeader';
// @ts-ignore;
import PdfPreviewDialog from '@/components/PdfPreviewDialog';
export default function RegulationManagement(props) {
  const {
    $w,
    style
  } = props;
  ...
  const loadRegulations = async () => {
    try {
      ...
      if (result && result.records) {
        ...
        // 提取分类（从所有记录中提取，不局限于当前页）
        const allCategoriesResult = await $w.cloud.callDataSource({
          dataSourceName: 'mc_regulations',
          methodName: 'wedaGetRecordsV2',
          params: {
            select: {
        ...
        const uniqueCategories = [...new Set(allCategoriesResult.records.map(r => r.category).filter(Boolean))];
        setCategories(uniqueCategories);
      }
    } catch (error) {
      console.error('加载规章制度数据失败:', error);
      toast({
        title: "加载失败",
        description: "无法加载规章制度数据",
        variant: "destructive"
      });
    } finally {
      setLoading(false);
  ...
  const handleDelete = async regulation => {
    ...
    if (window.confirm(`确定要删除《${regulation.fileName}》吗？此操作不可恢复。`)) {
      try {
        ...
        toast({
          title: "删除失败",
          description: error.message || "无法删除规章制度",
          variant: "destructive"
        });
      }
    }
      ...
      <div className="max-w-6xl mx-auto mt-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-600 mt-2">管理公司各项规章制度文件</p>
          </div>
        ...
        {!isAdmin && <Alert className="mb-4">
            ...
            <AlertDescription>
              您当前以普通用户身份查看，仅支持查看功能。如需上传、编辑或删除规章制度，请联系管理员。
            </AlertDescription>
          </Alert>}
              ...
              <div className="flex items-center space-x-4">
                ...
                setCurrentPage(1); // 切换分类时重置到第一页
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="安环体系">安环体系</SelectItem>
                  </SelectContent>
                </Select>
              </div>
        ...
        {/* 文件列表 - 使用regulations而不是filteredRegulations */}
        <Card>
          <CardHeader>
            <CardTitle>
              规章制度文件 ({totalCount})
          ...
          <CardContent>
            {loading ? <div className="space-y-3">
              ...
              length: 5
                ...
                <p className="text-gray-500">暂无规章制度文件</p>
                {isAdmin && <p className="text-sm text-gray-400 mt-2">点击"上传文件"来创建第一条规章制度</p>}
        ...
        {/* 上传对话框 */}
        {isAdmin && <RegulationUploadDialog open={showUploadDialog} onOpenChange={setShowUploadDialog} onSuccess={() => {
        ...
        {/* 批量上传对话框 */}
        {isAdmin && <RegulationBatchUploadDialog open={showBatchUploadDialog} onOpenChange={setShowBatchUploadDialog} onSuccess={() => {
