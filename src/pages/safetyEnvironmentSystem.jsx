...
// @ts-ignore;
import SafetyUploadDialog from '@/components/SafetyUploadDialog';
// @ts-ignore;
import SafetyBatchUploadDialog from '@/components/SafetyBatchUploadDialog';
export default function SafetyEnvironmentSystem(props) {
  const {
    $w,
    style
  } = props;
  ...
  const loadRegulations = async () => {
    try {
      ...
      const result = await $w.cloud.callDataSource({
        dataSourceName: 'mc_safety_environment_system',
      ...
      console.error('加载安环体系数据失败:', error);
      toast({
        title: "加载失败",
        description: "无法加载安环体系数据",
        variant: "destructive"
      });
    } finally {
      setLoading(false);
  ...
  const handleDelete = async regulation => {
    ...
    if (window.confirm(`确定要删除《${regulation.fileName}》吗？此操作不可恢复。`)) {
      try {
        ...
        toast({
          title: "删除失败",
          description: error.message || "无法删除安环体系文件",
          variant: "destructive"
        });
      }
    }
      ...
      <div className="max-w-6xl mx-auto mt-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-600 mt-2">管理公司安环体系文件</p>
          </div>
        ...
        {!isAdmin && <Alert className="mb-4">
            ...
            <AlertDescription>
              您当前以普通用户身份查看，仅支持查看功能。如需上传、编辑或删除安环体系文件，请联系管理员。
            </AlertDescription>
          </Alert>}
          ...
          <CardContent>
            {loading ? <div className="space-y-3">
              ...
              length: 5
                ...
                <p className="text-gray-500">暂无安环体系文件</p>
                {isAdmin && <p className="text-sm text-gray-400 mt-2">点击"上传文件"来创建第一条安环体系文件</p>}
        ...
        {/* 上传对话框 */}
        <SafetyUploadDialog open={showUploadDialog} onOpenChange={setShowUploadDialog} onSuccess={() => {
        setCurrentPage(1);
        loadRegulations();
      }} dataSourceName="mc_safety_environment_system" />

        {/* 批量上传对话框 */}
        <SafetyBatchUploadDialog open={showBatchUploadDialog} onOpenChange={setShowBatchUploadDialog} onSuccess={() => {
        setCurrentPage(1);
        loadRegulations();
      }} dataSourceName="mc_safety_environment_system" />
