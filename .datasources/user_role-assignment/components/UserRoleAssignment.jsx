...
export function UserRoleAssignment({
  ...
  const getAllUserPermissions = () => {
    ...
    const allPermissions = userRoles.reduce((acc, roleId) => {
  ...
  // 同步用户权限（合并所有角色的权限）
  const syncUserPermissions = async (userId, roleIds = null) => {
    setSyncing(true);
    setSyncStatus(null);
    setSyncMessage('');
    try {
      // 确定要同步的角色ID
      const rolesToSync = roleIds || userRoles;
      console.log('同步权限信息:', {
        userId,
        rolesToSync,
      ...
      if (rolesToSync.length === 0) {
        setSyncStatus('error');
        setSyncMessage('用户没有分配角色，无法同步权限');
        setDebugInfo(prev => ({
          ...prev,
          syncError: '用户没有角色',
          rolesToSync: []
      ...
      // 合并所有角色的权限
      const allPermissions = rolesToSync.reduce((acc, roleId) => {
        const rolePermissions = getRolePermissions(roleId);
        return [...acc, ...rolePermissions];
      }, []);
      ...
      if (uniquePermissions.length === 0) {
        setSyncStatus('error');
        setSyncMessage('所选角色没有配置权限，无法同步');
        setDebugInfo(prev => ({
          ...prev,
          syncError: '角色权限为空',
          uniquePermissions: []
      ...
      if (updateResult.count > 0) {
        ...
        setSyncMessage(`权限同步成功！已为用户同步 ${uniquePermissions.length} 个权限`);
        setDebugInfo(prev => ({
          ...prev,
          syncSuccess: true,
          permissionsSynced: uniquePermissions,
  ...
  const handleAssignRole = async () => {
    ...
    try {
      ...
      // 先分配角色（支持多角色）
      const newRoles = [...userRoles, selectedRole];
      const newRolesLevel = [...userRolesLevel, roleLevel];
      ...
      // 然后同步权限（合并所有角色的权限）
      const syncSuccess = await syncUserPermissions(userId, newRoles);
      if (syncSuccess) {
        toast({
          title: "角色分配成功",
  ...
  const handleRemoveRole = async roleId => {
    try {
      ...
      const newRoles = userRoles.filter((_, index) => index !== roleIndex);
      const newRolesLevel = userRolesLevel.filter((_, index) => index !== roleIndex);
      ...
      if (result.count > 0) {
        // 重新同步权限
        await syncUserPermissions(userId, newRoles);
        setSyncStatus('success');
        setSyncMessage('角色已移除，权限已更新');
        toast({
          title: "角色移除成功",
          description: `已移除 ${getRoleName(roleId)} 角色并更新权限`
        });
        await loadUserRoles(); // 重新加载用户数据
        if (onRoleAssigned) onRoleAssigned();
      } else {
